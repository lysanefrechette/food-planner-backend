...
export async function up(knex: Knex) {
  return knex.schema.createTable(tableName, (t) => {
    ...
    t.integer('day_time_id').notNullable().unsigned();

    t.foreign('day_time_id', 'fk_meal_day_time_id')
      .references('id')
      .inTable('time_of_day');

    t.integer('notification_id').nullable().unsigned();
    ...
    t.foreign('user_id', 'fk_meal_user_id').references('id').inTable('user');

    t.datetime('send_notification_at').nullable();

    t.tinyint('notification_sent').nullable();
  });
}
